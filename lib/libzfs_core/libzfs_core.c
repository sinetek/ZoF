    ...
    nvlist_t *source, nvlist_t **resultp)
{
	...
	while (zcmd_ioctl(g_fd, ioc, &zc) != 0) {
		/*
...
lzc_promote(const char *fsname, char *snapnamebuf, int snapnamelen)
{
	...
	if (zcmd_ioctl(g_fd, ZFS_IOC_PROMOTE, &zc) != 0) {
		int error = errno;
...
lzc_rename(const char *source, const char *target)
{
	...
	error = zcmd_ioctl(g_fd, ZFS_IOC_RENAME, &zc);
	if (error != 0)
		error = errno;
	return (error);
}
...
lzc_exists(const char *dataset)
{
	...
	return (zcmd_ioctl(g_fd, ZFS_IOC_OBJSET_STATS, &zc) == 0);
}
    ...
    nvlist_t **errors)
{
	...
	*atp = '\0';
	...
	if (resumable || raw || wkeydata != NULL || payload) {
		...
		error = zcmd_ioctl(g_fd, ZFS_IOC_RECV, &zc);
		if (error != 0) {
			error = errno;
		} else {
			if (read_bytes != NULL)
