...
#if defined(_KERNEL)
/* BEGIN CSTYLED */
ZFS_MODULE_PARAM(zfs_prefetch, zfs_prefetch_, disable, UINT, ZMOD_RW, "Disable all ZFS prefetching");

ZFS_MODULE_PARAM(zfs_prefetch, zfetch_, max_streams, UINT, ZMOD_RW, "Max number of streams per zfetch");

ZFS_MODULE_PARAM(zfs_prefetch, zfetch_, min_sec_reap, UINT, ZMOD_RW, "Min time before stream reclaim");

ZFS_MODULE_PARAM(zfs_prefetch, zfetch_, max_distance, UINT, ZMOD_RW,
	"Max bytes to prefetch per stream (default 8MB)");

ZFS_MODULE_PARAM(zfs_prefetch, zfetch_, array_rd_sz, UQUAD, ZMOD_RW, "Number of bytes in a array_read");
/* END CSTYLED */
#endif
