...
dsl_scan_setup_sync(void *arg, dmu_tx_t *tx)
{
	...
	spa_history_log_internal(spa, "scan setup", tx,
	    ...
	    *funcp, (longlong_t)scn->scn_phys.scn_min_txg, (longlong_t)scn->scn_phys.scn_max_txg);
}
...
ZFS_MODULE_PARAM(zfs, zfs_, scrub_min_time_ms, UINT, ZMOD_RW, "Min millisecs to scrub per txg");

ZFS_MODULE_PARAM(zfs, zfs_, obsolete_min_time_ms, UINT, ZMOD_RW, "Min millisecs to obsolete per txg");

ZFS_MODULE_PARAM(zfs, zfs_, free_min_time_ms, UINT, ZMOD_RW, "Min millisecs to free per txg");

ZFS_MODULE_PARAM(zfs, zfs_, resilver_min_time_ms, UINT, ZMOD_RW, "Min millisecs to resilver per txg");

ZFS_MODULE_PARAM(zfs, zfs_, scan_suspend_progress, UINT, ZMOD_RW,
	"Set to prevent scans from progressing");

ZFS_MODULE_PARAM(zfs, zfs_, no_scrub_io, UINT, ZMOD_RW, "Set to disable scrub I/O");

ZFS_MODULE_PARAM(zfs, zfs_, no_scrub_prefetch, UINT, ZMOD_RW, "Set to disable scrub prefetching");

/* CSTYLED */
ZFS_MODULE_PARAM(zfs, zfs_, async_block_max_blocks, UQUAD, ZMOD_RW,
	"Max number of blocks freed in one txg");

ZFS_MODULE_PARAM(zfs, zfs_, free_bpobj_enabled, UINT, ZMOD_RW,  "Enable processing of the free_bpobj");

ZFS_MODULE_PARAM(zfs, zfs_, scan_mem_lim_fact, UINT, ZMOD_RW, "Fraction of RAM for scan hard limit");

ZFS_MODULE_PARAM(zfs, zfs_, scan_issue_strategy, UINT, ZMOD_RW,
	"IO issuing strategy during scrubbing. 0 = default, 1 = LBA, 2 = size");

ZFS_MODULE_PARAM(zfs, zfs_, scan_legacy, UINT, ZMOD_RW, "Scrub using legacy non-sequential method");
