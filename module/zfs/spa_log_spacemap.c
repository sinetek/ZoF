...
#if defined(_KERNEL)
/* BEGIN CSTYLED */
ZFS_MODULE_PARAM(zfs, zfs_, unflushed_max_mem_amt, UQUAD, ZMOD_RW,
    "Specific hard-limit in memory that ZFS allows to be used for "
    "unflushed changes");

ZFS_MODULE_PARAM(zfs, zfs_, unflushed_max_mem_ppm, UQUAD, ZMOD_RW,
...
ZFS_MODULE_PARAM(zfs, zfs_, unflushed_log_block_max, UQUAD, ZMOD_RW,
    "Hard limit (upper-bound) in the size of the space map log "
    "in terms of blocks.");

ZFS_MODULE_PARAM(zfs, zfs_, unflushed_log_block_min, UQUAD, ZMOD_RW,
    "Lower-bound limit for the maximum amount of blocks allowed in "
    "log spacemap (see zfs_unflushed_log_block_max)");

ZFS_MODULE_PARAM(zfs, zfs_, unflushed_log_block_pct, UQUAD, ZMOD_RW,
...
ZFS_MODULE_PARAM(zfs, zfs_, max_log_walking, UQUAD, ZMOD_RW,
    "The number of past TXGs that the flushing algorithm of the log "
    "spacemap feature uses to estimate incoming log blocks");

ZFS_MODULE_PARAM(zfs, zfs_, max_logsm_summary_length, UQUAD, ZMOD_RW,
    "Maximum number of rows allowed in the summary of "
    "the spacemap log");

ZFS_MODULE_PARAM(zfs, zfs_, min_metaslabs_to_flush, UQUAD, ZMOD_RW,
    "Minimum number of metaslabs to flush per dirty TXG");

ZFS_MODULE_PARAM(zfs, zfs_, keep_log_spacemaps_at_export, UINT, ZMOD_RW,
